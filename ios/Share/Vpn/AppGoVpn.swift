...
@objcMembers
class AppGoVpn: NSObject {
  static let shared = AppGoVpn()
  ...
  private let connectivity: AppGoConnectivity
  ...
  //  - cordova-plugin-AppGo/apple/vpn/PacketTunnelProvider.h#NS_ENUM
  //  - cordova-plugin-AppGo/AppGoPlugin.js#ERROR_CODE
  //  - www/model/errors.ts
  @objc
  ...
  // Starts a VPN connection as specified in the AppGoConnection object.
  func start(_ connection: AppGoConnection, _ completion: @escaping (Callback)) {
  ...
  func startLastSuccessfulConnection(_ completion: @escaping (Callback)) {
    ...
    self.startVpn(AppGoConnection(), isAutoConnect: true, completion)
  }
  ...
  func isReachable(_ connection: AppGoConnection, _ completion: @escaping Callback) {
    ...
    if isVpnConnected() {
      // All the device's traffic, including the AppGo app, go through the VpnExtension process.
  ...
  private func startVpn(
      _ connection: AppGoConnection, isAutoConnect: Bool, _ completion: @escaping(Callback)) {
  ...
  // Adds a VPN configuration to the user preferences if no AppGo profile is present. Otherwise
  ...
  private func setupVpn(completion: @escaping(Error?) -> Void) {
    NETunnelProviderManager.loadAllFromPreferences() { (managers, error) in
      ...
      if let managers = managers, managers.count > 0 {
        ...
        if config.providerBundleIdentifier != AppGoVpn.kVpnExtensionBundleId {
            manager.removeFromPreferences() { error in
                self.createTunnelProviderManager() { _ in
    ...
    private func createTunnelProviderManager(completion: @escaping(Error?) -> Void) {
        let config = NETunnelProviderProtocol()
        config.providerBundleIdentifier = AppGoVpn.kVpnExtensionBundleId
        config.serverAddress = "AppGo"
