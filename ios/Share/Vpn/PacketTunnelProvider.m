...
@property (nonatomic) AppGoConnection *connection;
@property (nonatomic) AppGoConnectionStore *connectionStore;
@end
...
- (id)init {
  self = [super init];
#if TARGET_OS_IPHONE
  NSString *appGroup = @"group.com.appgo.appgoios";
#else
  NSString *appGroup = @"QT8Z3Q9V3A.com.appgo.appgox";
#endif
  ...
  _connectionStore = [[AppGoConnectionStore alloc] initWithAppGroup:appGroup];
...
- (void)startTunnelWithOptions:(NSDictionary *)options
  ...
  if (options == nil) {
    ...
    // TODO(alalama): l10n
    [self displayMessage:@"Please use the AppGo app to connect."
  ...
  AppGoConnection *connection = [self retrieveConnection:options];
  if (connection == nil) {
  ...
  // still be unusable, but at least the user will have a visual indication that AppGo is the
...
- (void)handleAppMessage:(NSData *)messageData completionHandler:(void (^)(NSData *))completionHandler {
  ...
  if ([kActionStart isEqualToString:action] || [kActionRestart isEqualToString:action]) {
    ...
    if ([kActionRestart isEqualToString:action]) {
      self.connection = [[AppGoConnection alloc] initWithId:message[kMessageKeyConnectionId]
...
// Creates a AppGoConnection from options supplied in |config|, or retrieves the last working
...
- (AppGoConnection *)retrieveConnection:(NSDictionary *)config {
  AppGoConnection *connection;
  if (config != nil && !config[kMessageKeyOnDemand]) {
    connection = [[AppGoConnection alloc] initWithId:config[kMessageKeyConnectionId] config:config];
...
- (NEPacketTunnelNetworkSettings *) getTunnelNetworkSettings {
  ...
  NEIPv4Settings *ipv4Settings = [[NEIPv4Settings alloc] initWithAddresses:@[@"192.168.21.2"]
  ...
  NEIPv6Settings *ipv6Settings = [[NEIPv6Settings alloc] initWithAddresses:@[@"fd66:f83a:c650::2"]
  ...
  NEPacketTunnelNetworkSettings *settings = [[NEPacketTunnelNetworkSettings alloc]
                                             initWithTunnelRemoteAddress:@"192.168.21.1"];
