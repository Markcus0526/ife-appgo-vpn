...
package com.appgo.appgopro.acl

import android.support.v7.util.SortedList
import android.util.Log
import com.appgo.appgopro.AppGoApplication.Companion.app
import com.appgo.appgopro.utils.Subnet
import com.appgo.appgopro.utils.asIterable
import java.io.File
...
class Acl {
    companion object {
        ...
        fun getFile(id: String) = File(app.deviceContext.filesDir, id + ".acl")
...
//                null else value.toString())
        ...
        fun save(id: String, acl: Acl) = Acl.Companion.getFile(id).writeText(acl.toString())
    }
    ...
    private open class DefaultSorter<T : Comparable<T>> : Acl.BaseSorter<T>() {
        override fun compareNonNull(o1: T, o2: T): Int = o1.compareTo(o2)
    }
    private object StringSorter : Acl.DefaultSorter<String>()
    private object SubnetSorter : Acl.DefaultSorter<Subnet>()
    private object URLSorter : Acl.BaseSorter<URL>() {
        private val ordering = compareBy<URL>({ it.host }, { it.port }, { it.file }, { it.protocol })
        override fun compareNonNull(o1: URL, o2: URL): Int = Acl.URLSorter.ordering.compare(o1, o2)
    }

    val hostnames = SortedList(String::class.java, Acl.StringSorter)
    val subnets = SortedList(Subnet::class.java, Acl.SubnetSorter)
    val urls = SortedList(URL::class.java, Acl.URLSorter)
    var bypass = false
    ...
    fun fromReader(reader: Reader, defaultBypass: Boolean = false): Acl {
        ...
        val proxyHostnames by lazy { SortedList(String::class.java, Acl.StringSorter) }
        val bypassHostnames by lazy { SortedList(String::class.java, Acl.StringSorter) }
        val bypassSubnets by lazy { SortedList(Subnet::class.java, Acl.SubnetSorter) }
        val proxySubnets by lazy { SortedList(Subnet::class.java, Acl.SubnetSorter) }
        ...
        reader.useLines {
            for (line in it) {
                ...
                val url = Acl.Companion.networkAclParser.matchEntire(blocks.getOrElse(1, {""}))?.groupValues?.getOrNull(1)
    ...
    fun fromId(id: String): Acl = try {
        fromReader(Acl.Companion.getFile(id).bufferedReader())
    ...
    fun flatten(depth: Int): Acl {
        if (depth > 0) for (url in urls.asIterable()) {
            ...
            if (bypass != child.bypass) {
                Log.w(Acl.Companion.TAG, "Imported network ACL has a conflicting mode set. " +
