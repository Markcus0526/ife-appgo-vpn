...
There are no arbitrary restrictions/requirements on package name, component name and content
 ...
 name, use `com.appgo.appgopro.plugin.$PLUGIN_ID` if it only contains a single plugin to prevent
 ...
 `obfs-local` would probably be `com.appgo.appgopro.plugin.obfs_local`.
...
* MUST have action: `com.appgo.appgopro.plugin.ACTION_CONFIGURE`;
* MUST have category: `android.intent.category.DEFAULT`;
* MUST be able to receive data URI `plugin://com.appgo.appgopro/$PLUGIN_ID`;
* SHOULD parse string extra `com.appgo.appgopro.plugin.EXTRA_OPTIONS` (all options as a single
...
* On finish, it SHOULD return one of the following results:
  - `RESULT_OK = 0`: In this case it MUST return the data Intent with the new
    `com.appgo.appgopro.plugin.EXTRA_OPTIONS`;
...
This corresponds to `com.appgo.appgopro.plugin.ConfigurationActivity` in the plugin library.
...
<manifest>
    ...
    <application>
        ...
        <activity android:name=".ConfigActivity">
            <intent-filter>
                <action android:name="com.appgo.appgopro.plugin.ACTION_CONFIGURE"/>
                <category android:name="android.intent.category.DEFAULT"/>
                <data android:scheme="plugin"
                      android:host="com.appgo.appgopro"
...
* MUST have action: `com.appgo.appgopro.plugin.ACTION_HELP`;
* MUST have category: `android.intent.category.DEFAULT`;
* MUST be able to receive data URI `plugin://com.appgo.appgopro/$PLUGIN_ID`;
* CAN parse string extra `com.appgo.appgopro.plugin.EXTRA_OPTIONS` and display some more
  relevant information;
* SHOULD either:
  - Be invisible and return help message with CharSequence extra
    `com.appgo.appgopro.plugin.EXTRA_HELP_MESSAGE` in the data intent with `RESULT_OK`; (in this
...
This corresponds to `com.appgo.appgopro.plugin.HelpActivity` or
 `com.appgo.appgopro.plugin.HelpCallback` in the plugin library. Here's what a proper help
...
<manifest>
    ...
    <application>
        ...
        <activity android:name=".HelpActivity">
            <intent-filter>
                <action android:name="com.appgo.appgopro.plugin.ACTION_HELP"/>
                <category android:name="android.intent.category.DEFAULT"/>
                <data android:scheme="plugin"
                      android:host="com.appgo.appgopro"
                      android:path="/$PLUGIN_ID"/>
...
* MUST have an intent filter with action `com.appgo.appgopro.plugin.ACTION_NATIVE_PLUGIN`;
  (used for discovering plugins)
* MUST have meta-data `com.appgo.appgopro.plugin.id` with string value `$PLUGIN_ID`;
* MUST have an intent filter with action `com.appgo.appgopro.plugin.ACTION_NATIVE_PLUGIN` and
  data `plugin://com.appgo.appgopro/$PLUGIN_ID`; (used for configuring plugin)
* CAN have meta-data `com.appgo.appgopro.plugin.default_config` with string value, default is
  empty;
...
* MUST implement `openFile` that for files returned in `query`, `openFile` with `mode = "r"` returns
  ...
  `content://com.appgo.appgopro.plugin.kcptun/kcptun`.

This corresponds to `com.appgo.appgopro.plugin.NativePluginProvider` in the plugin library.
...
<manifest>
    ...
    <application>
        ...
        <provider android:name=".BinaryProvider"
            ...
            <intent-filter>
                <action android:name="com.appgo.appgopro.plugin.ACTION_NATIVE_PLUGIN"/>
            </intent-filter>
            <intent-filter>
                <action android:name="com.appgo.appgopro.plugin.ACTION_NATIVE_PLUGIN"/>
                <data android:scheme="plugin"
            ...
            <meta-data android:name="com.appgo.appgopro.plugin.id"
                       android:value="$PLUGIN_ID"/>
            <meta-data android:name="com.appgo.appgopro.plugin.default_config"
...
* MUST implement `call` that returns absolute path to the entry executable as
  `com.appgo.appgopro.plugin.EXTRA_ENTRY` when having `method = "shadowsocks:getExecutable"`;
  (`com.appgo.appgopro.plugin.EXTRA_OPTIONS` is provided in extras as well just in case you
  need them)
...
```
<meta-data android:name="com.appgo.appgopro.plugin.version"
           android:value="0.0.2"/>
```
