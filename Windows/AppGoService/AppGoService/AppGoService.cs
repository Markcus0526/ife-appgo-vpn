// Copyright 2018 The AppGoService Authors
//
...
/*
 * Windows Service, part of the AppGo Windows client, to configure routing.
 ...
 *
 */
namespace AppGoService {
  public partial class AppGoService : ServiceBase {
    private const string EVENT_LOG_SOURCE = "AppGoService";
    private const string EVENT_LOG_NAME = "Application";
    private const string PIPE_NAME = "AppGoServicePipe"; // Must be kept in sync with electron.
    private const string TAP_DEVICE_NAME = "AppGo";
    ...
    protected override void OnStart(string[] args) {
      eventLog.WriteEntry("AppGoService starting");
    ...
    protected override void OnStop() {
      eventLog.WriteEntry("AppGoService stopping");
      DestroyPipe();
    ...
    public void ConfigureRouting(string routerIp, string proxyIp, bool isAutoConnect) {
      ...
      //      netsh interface ip set interface AppGo metric=auto
      ...
      try {
        ...
        eventLog.WriteEntry($"netsh interface ip set interface {TAP_DEVICE_NAME} metric=0", EventLogEntryType.Information);
      } catch (Exception e) {
      ...
      try {
        ...
        eventLog.WriteEntry($"SetGatewayProperties fails {e.Message}", EventLogEntryType.Error);
      }

      if (gatewayIp != null) {
        try {
          AddProxyRoute(proxyIp, gatewayIp.ToString(), gatewayInterfaceName);
          eventLog.WriteEntry($"AddProxyRoute {proxyIp} {gatewayIp.ToString()} {gatewayInterfaceName}", EventLogEntryType.Information);
        } catch (Exception e) {
    ...
    private void AddProxyRoute(string proxyIp, string systemGatewayIp, string interfaceName) {
      try {
        ...
        // server from a previous run of AppGoService which ResetRouting could
    ...
    private void AddIpv4Redirect(string routerIp) {
      try {
        foreach (string subnet in IPV4_SUBNETS) {
          ...
          eventLog.WriteEntry($"netsh interface ipv4 add route {subnet} nexthop={routerIp} interface={TAP_DEVICE_NAME} metric=0", EventLogEntryType.Information);
        }
      } catch (Exception e) {
    ...
    // AppGo does not currently support IPv6, so we resort to disabling it while the VPN is active to
    ...
    // interface iff we think we can modify the routing to route via AppGo.
    ...
    private NetworkInterface GetSystemIpv4Gateway() {
      ...
      //
      // Notes:
      //  - Ignore AppGo as in certain rare situations - tun2socks crash? - it can
    ...
    private void NetworkAddressChanged(object sender, EventArgs evt) {
      if (proxyIp == null) {
        eventLog.WriteEntry("Network change but AppGo is not connected, nothing to do.");
        return;
      }
