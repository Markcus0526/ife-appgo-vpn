...
class AppGoConnection(private val instance: com.appgo.appgopro.AppGoConnection.Interface) : ServiceConnection {
    companion object {
        private val connections = WeakHashMap<com.appgo.appgopro.AppGoConnection.Interface, com.appgo.appgopro.AppGoConnection>()
    }

    interface Interface : IBinder.DeathRecipient {
        val serviceCallback: IAppGoServiceCallback? get() = null
        val connection: com.appgo.appgopro.AppGoConnection
            get() = connections.getOrPut(this, { com.appgo.appgopro.AppGoConnection(this) })
        val listenForDeath get() = false

        fun onServiceConnected(service: IAppGoService) { }
        /**
    ...
    var service: IAppGoService? = null

    override fun onServiceConnected(name: ComponentName?, binder: IBinder) {
        this.binder = binder
        if (instance.listenForDeath) binder.linkToDeath(instance, 0)
        val service = IAppGoService.Stub.asInterface(binder)!!
