...
import com.appgo.appgopro.utils.Key
import com.appgo.appgopro.utils.parsePort
...
class Profile : Serializable {
            ...
            try {
                if (uri.userInfo == null) {
                    ...
                    if (match != null) {
                        ...
                        profile.ip = match.groupValues[3]
                    ...
                    if (match != null) {
                        ...
                        try {
                            ...
                            profile.ip = javaURI.host ?: ""
                            if (profile.ip.firstOrNull() == '[' && profile.ip.lastOrNull() == ']')
                                profile.ip = profile.ip.substring(1, profile.ip.length - 1)
    ...
    val formattedAddress get() = (if (ip.contains(":")) "[%s]:%d" else "%s:%d").format(ip, remotePort)
        ...
        val builder = Uri.Builder()
                ...
                .encodedAuthority("%s@%s:%d".format(Locale.ENGLISH,
                        Base64.encodeToString("%s:%s".format(Locale.ENGLISH, method, password).toByteArray(),
                                Base64.NO_PADDING or Base64.NO_WRAP or Base64.URL_SAFE),
                        if (ip.contains(':')) "[$ip]" else ip, remotePort))
    ...
    fun serialize() {
        ...
        DataStore.privateStore.putString(Key.host, ip)
    ...
    fun deserialize() {
        ...
        ip = DataStore.privateStore.getString(Key.host) ?: ""
