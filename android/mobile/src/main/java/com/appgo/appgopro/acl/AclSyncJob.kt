...
class AclSyncJob(private val route: String) : Job() {
    companion object : JobCreator {
        ...
        override fun create(tag: String): AclSyncJob? {
            ...
            return when (parts[0]) {
                AclSyncJob.Companion.TAG -> AclSyncJob(parts[1])
                else -> {
                    Log.w(AclSyncJob.Companion.TAG, "Unknown job tag: " + tag)
                    null
                }
            }
        }

        fun schedule(route: String): Int = JobRequest.Builder(AclSyncJob.Companion.TAG + ':' + route)
    ...
    override fun onRunJob(params: Params): Result = try {
        val acl = URL("https://shadowsocks.org/acl/android/v1/$route.acl").openStream().bufferedReader()
                .use { it.readText() }
        Acl.Companion.getFile(route).printWriter().use { it.write(acl) }
        Result.SUCCESS
    } catch (e: IOException) {
        e.printStackTrace()
        if (params.failureCount < AclSyncJob.Companion.MAX_RESCHEDULE) Result.RESCHEDULE else Result.FAILURE
